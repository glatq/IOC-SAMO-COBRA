...
class SRD:
    def __init__(self):
        self.lower = np.array([2.6, 0.7, 17, 7.3, 7.3, 2.9, 5])
        self.upper = np.array([3.6, 0.8, 28, 8.3, 8.3, 3.9, 5.5])
        self.nConstraints = 11
        self.nObj = 2
        self.ref = np.array([7000, 1700])
        self.nadir = np.array([5781.90, 1690.16])
        self.cheapConstr = [
            True,
            True,
            True,
    ...
    def evaluate(self, x):
        ...
        x7 = x[6]
        fweight = (
            0.7854 * (x1 * (x2**2) * ((10 * (x3**2) / 3) + (14.933 * x3) - 43.0934))
            - 1.508 * x1 * ((x6**2) + (x7**2))
            + 7.477 * ((x6**3) + (x7**3))
        ...
        g10 = 1.9 - x4 + (1.5 * x6)
        g11 = 1.9 - x5 + (1.1 * x7)
        g5 = (np.sqrt(((745 * x4) / (x2 * x3)) ** 2 + 1.69 * 10**7)/ (110 * (x6**3))) - 1
        g6 = (np.sqrt(((745 * x5) / (x2 * x3)) ** 2 + 1.575* 10**8) / (85 * (x7**3))) - 1

        return np.array([fweight, fstress]), np.array(
            [27 * g1, 397.5 * g2, 1.93 * g3, 1.93 * g4, g5, g6, g7, g8, g9, g10, g11]
        )
    ...
    def cheap_evaluate(self, x):
        ...
        if self.cheapObj[1]:
            fstress = np.sqrt((((745 * x4) / (x2 * x3)) ** 2) + (1.691e7)) / (
                0.1 * (x6**3)
            )
        else:
            fstress = np.nan
        ...
        if self.cheapConstr[7]:
            g8 = 1.9 - x4 + (1.5 * x6)
        else:
            g8 = np.nan

        if self.cheapConstr[8]:
            g9 = 1.9 - x5 + (1.1 * x7)
        else:
            g9 = np.nan

        if self.cheapConstr[9]:
            g10 = (
                (np.sqrt(((745 * x4) / (x2 * x3)) ** 2) + 1.691**7) / (0.1 * (x6**3))
            ) - 1300
        else:
            g10 = np.nan

        if self.cheapConstr[10]:
            g11 = (
                (np.sqrt(((745 * x5) / (x2 * x3)) ** 2) + 1.5751**8) / (0.1 * (x7**3))
            ) - 1100
