...
def cheap_SAMO_COBRA_Init(
    problem,
    nCores=7,
    computeStartingPoints=16,
    feval=None,
    infillCriteria="PHV",
    batch=1,
    oneShot=False,
    ...
    if hasattr(problem, "nConstraints"):
        ...
        raise ValueError(
            "Problem class must at least have the number of constraints defined in nConstraints parameter"
        )
    ...
    if hasattr(problem, "ref"):
        ...
        raise ValueError(
            "Problem class must at least have a reference point defined in ref prameter"
        )
    ...
    if hasattr(problem, "lower"):
        ...
        raise ValueError(
            "Problem class must at least have lower bound for decision variables in lower object"
        )
    ...
    if hasattr(problem, "upper"):
        ...
        raise ValueError(
            "Problem class must at least have upper boudn for decision variables in upper object"
        )
    ...
    if hasattr(problem, "evaluate"):
        fnn = partial(
            ...
            lower=originalL,
            upper=originalU,
        )  # rescaleWrapper(fn,originalL,originalU,newlower,newupper)
    else:
    ...
    cobra["bestPredictor"] = [
        {
            "objKernel": [cobra["RBFmodel"][0]] * nObj,
            "objLogStr": ["Standardized"] * nObj,
            "conKernel": [cobra["RBFmodel"][0]] * nConstraints,
            "conLogStr": ["Rescaled"] * nConstraints,
        }
    ]
    ...
    for kernel in cobra["RBFmodel"]:
        for obji in range(cobra["nObj"]):
            ...
            surrogateErrors["OBJ" + str(obji) + "PLOG" + kernel] = [0] * cobra[
                "initDesPoints"
            ]
        for coni in range(cobra["nConstraints"]):
            surrogateErrors["CON" + str(coni) + kernel] = [0] * cobra["initDesPoints"]
            surrogateErrors["CON" + str(coni) + "PLOG" + kernel] = [0] * cobra[
                "initDesPoints"
            ]
