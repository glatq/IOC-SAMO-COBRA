...
def fitRBF(
    phi, U, ptail=True, squares=False, xp=None, rbftype="THINPLATESPLINE", width=1
):
    """
...
def interpRBF(x, rbfModel, uncertainty=False):
    ...
    ed = distLine(
        x, rbfModel["xp"]
    )  # euclidean distance of x to all xp, ed is vector of length nrow(xp)
    ...
    if uncertainty:
        uncertainty_value = rbfModel["uncertainty0Val"] - np.matmul(
            np.matmul(lhs, rbfModel["phiinv"]), lhs.T
        )
