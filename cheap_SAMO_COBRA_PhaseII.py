...
def getConstraintPrediction(
    x,
    surrogateModels,
    bestPredictor,
    nConstraints,
    GresPlogRescaledDivider,
    GresRescaledDivider,
    fnCheap,
    cheapCon,
    EPS=None,
        ...
        if cheapCon[coni]:
            ...
            if conLogStr == "PLOGRescaled":
                ...
                constraintPrediction = (
                    GresPlogRescaledDivider[coni] * constraintPrediction
                )
            else:
                constraintPrediction = interpRBF(np.array(x), surrogate)
...
def getEquallyImportantConstraintPrediction(
    x,
    surrogateModels,
    bestPredictor,
    nConstraints,
    ...
    for coni in range(nConstraints):
        if cheapCon[coni]:
            if EPS is None:
                constraintPredictions[coni] = -1 * (
                    cheap_constr[coni] / GresRescaledDivider[coni]
                )
            else:
                constraintPredictions[coni] = -1 * (
                    (cheap_constr[coni] / GresRescaledDivider[coni]) + EPS[coni] ** 2
                )
        else:
            conKernel = bestPredictor["conKernel"][coni]
            conLogStr = bestPredictor["conLogStr"][coni]
            ...
            if EPS is None:
                ...
                constraintPredictions[coni] = -1 * (
                    constraintPrediction + EPS[coni] ** 2
                )
...
def gCOBRA(
    ...
    constraintPrediction = getEquallyImportantConstraintPrediction(
        cliped_x,
        surrogateModels,
        bestPredictor,
        nConstraints,
    ...
    if any(np.isnan(constraintPrediction)):
        warnings.warn(
            "gCOBRA: constraintPrediction value is NaN, returning Inf",
            DeprecationWarning,
        )
...
def get_potentialSolution(
    ...
    nObj,
    infillCriteria,
    FresPlogStandardizedStd,
    FresStandardizedStd,
    FresPlogStandardizedMean,
    FresStandardizedMean,
    fnCheap,
    cheapObj,
):
    cliped_xu = np.minimum(x, upper)
        ...
        if cheapObj[obji]:
            ...
            if objLogStr == "PLOGStandardized":
                if infillCriteria == "PHV":
                    ...
                    potsol = (
                        potsol * FresPlogStandardizedStd[obji]
                        + FresPlogStandardizedMean[obji]
                    )
                ...
                if infillCriteria == "PHV":
                    potsol = interpRBF(x, surrogate, uncertainty=False)
                    potsol = (
                        potsol * FresStandardizedStd[obji] + FresStandardizedMean[obji]
                    )
                elif infillCriteria == "SMS":
                    potsol, uncertainty = interpRBF(x, surrogate, uncertainty=True)
                    potsol = (
...
def compute_infill_criteria_score(
    x,
    lower,
    upper,
    surrogateModels,
    ...
    potentialSolution = get_potentialSolution(
        x,
        lower,
        upper,
        surrogateModels,
...
def batch_infill_criteria_score(
    x,
    batch,
    lower,
    upper,
    ...
    for potentialSolution in tempPF_dominated:
        if not any(
            np.sum(np.equal(potentialSolution, paretoFrontier), axis=1)
            == paretoFrontier.shape[1]
        ):
            logicBool = np.all(tempPF_pf <= potentialSolution, axis=1)
            penalty = np.max(
                -1 + np.prod(1 + (potentialSolution - tempPF_pf[logicBool]), axis=1)
            )
            penalties += penalty
...
def pool_job(
    ...
    subMin = minimize(
        criteria_function, xStart, constraints=cons, options=opts, method="COBYLA"
    )
...
def trainSurrogate(
    ...
    for g in GresRescaled:
        if not cheapConstr[gi]:
            surrogateModel["Constraints"]["Rescaled"].append(
                trainRBF(A, g, ptail=True, squares=True, smooth=0.00, rbftype=kernel)
            )
        else:
    ...
    for g in GresPlogRescaled:
        if not cheapConstr[gi]:
            surrogateModel["Constraints"]["PLOGRescaled"].append(
                trainRBF(A, g, ptail=True, squares=True, smooth=0.00, rbftype=kernel)
            )
        else:
    ...
    for f in FresStandardized:
        if not cheapObj[fi]:
            surrogateModel["Objectives"]["Standardized"].append(
                trainRBF(A, f, ptail=True, squares=True, smooth=0.00, rbftype=kernel)
            )
        else:
    ...
    for f in FresPlogStandardized:
        if not cheapObj[fi]:
            surrogateModel["Objectives"]["PLOGStandardized"].append(
                trainRBF(A, f, ptail=True, squares=True, smooth=0.00, rbftype=kernel)
            )
        else:
...
def cheap_SAMO_COBRA_PhaseII(cobra):
    ...
    if cobra["hypervolumeProgress"] is None:
        raise ValueError(
            "cobraPhaseII: cobra['hypervolumeProgress'] is None! First run smscobraInit"
        )
    ...
    if n == cobra["initDesPoints"]:
        ...
        cobra["optimizerConvergence"] = np.ones(
            cobra["initDesPoints"]
        )  # vector to store optimizer convergence
        feval = np.empty(cobra["initDesPoints"])
        feval[:] = np.nan

    if n >= cobra["feval"]:
        raise ValueError(
            "ERROR! Number of function evaluations after initialization is larger than total allowed evaluations"
        )
    ...
    def kfold_best_predictor(cobra, pool):
        ...
        for foldi in range(folds):
            ...
            trainCobra["FresStandardized"] = cobra["FresStandardized"][
                trainIndicator % folds != foldi
            ]
            trainCobra["FresPlogStandardized"] = cobra["FresPlogStandardized"][
                trainIndicator % folds != foldi
            ]
            trainCobra["GresRescaled"] = cobra["GresRescaled"][
                trainIndicator % folds != foldi
            ]
            trainCobra["GresPlogRescaled"] = cobra["GresPlogRescaled"][
                trainIndicator % folds != foldi
            ]
            ...
            testCobra["FresStandardized"] = cobra["FresStandardized"][
                trainIndicator % folds == foldi
            ]
            testCobra["FresPlogStandardized"] = cobra["FresPlogStandardized"][
                trainIndicator % folds == foldi
            ]
            testCobra["GresRescaled"] = cobra["GresRescaled"][
                trainIndicator % folds == foldi
            ]
            testCobra["GresPlogRescaled"] = cobra["GresPlogRescaled"][
                trainIndicator % folds == foldi
            ]
                ...
                for obji in range(cobra["nObj"]):
                    for kernel in cobra["RBFmodel"]:
                        ...
                        if not cobra["cheapObj"][obji]:
                            surrogatePlog = surrogateModels[kernel]["Objectives"][
                                "PLOGStandardized"
                            ][obji]
                            plogSol = interpRBF(x, surrogatePlog)
                            plogSol = (
                                plogSol * cobra["FresPlogStandardizedStd"][obji]
                                + cobra["FresPlogStandardizedMean"][obji]
                            )
                            plogSol = plogReverse(plogSol)
                            surrogateErrors["OBJ" + str(obji) + "PLOG" + kernel].append(
                                (plogSol - yTrue[obji]) ** 2
                            )

                            surrogate = surrogateModels[kernel]["Objectives"][
                                "Standardized"
                            ][obji]
                            sol = interpRBF(x, surrogate)
                            sol = (
                                sol * cobra["FresStandardizedStd"][obji]
                                + cobra["FresStandardizedMean"][obji]
                            )
                            surrogateErrors["OBJ" + str(obji) + kernel].append(
                                (sol - yTrue[obji]) ** 2
                            )
                        else:
                            surrogateErrors["OBJ" + str(obji) + "PLOG" + kernel].append(
                ...
                for coni in range(cobra["nConstraints"]):
                    for kernel in cobra["RBFmodel"]:
                        if not cobra["cheapCon"][coni]:
                            surrogatePlog = surrogateModels[kernel]["Constraints"][
                                "PLOGRescaled"
                            ][coni]
                            plogSol = interpRBF(x, surrogatePlog)
                            plogSol = plogReverse(plogSol)
                            plogSol = cobra["GresPlogRescaledDivider"][coni] * plogSol
                            surrogateErrors["CON" + str(coni) + "PLOG" + kernel].append(
                                (plogSol - conTrue[coni]) ** 2
                            )

                            surrogate = surrogateModels[kernel]["Constraints"][
                                "Rescaled"
                            ][coni]
                            sol = interpRBF(x, surrogate)
                            sol = cobra["GresRescaledDivider"][coni] * sol
                            surrogateErrors["CON" + str(coni) + kernel].append(
                                (sol - conTrue[coni]) ** 2
                            )
                        else:
                            surrogateErrors["CON" + str(coni) + "PLOG" + kernel].append(
            ...
            if not cobra["cheapObj"][obji]:
                for kernel in cobra["RBFmodel"]:
                    if (
                        np.sqrt(
                            np.mean(
                                np.array(surrogateErrors["OBJ" + str(obji) + kernel])
                            )
                        )
                        < minScore
                    ):
            ...
            if not cobra["cheapCon"][coni]:
                for kernel in cobra["RBFmodel"]:
                    if (
                        np.sqrt(
                            np.mean(
                                np.array(surrogateErrors["CON" + str(coni) + kernel])
                            )
                        )
                        < minScore
                    ):
    ...
    def define_best_predictor(x, yTrue, conTrue, surrogateModels, cobra):
            ...
            for obji in range(cobra["nObj"]):
                for kernel in cobra["RBFmodel"]:
                    if not cobra["cheapObj"][obji]:
                        ...
                        plogSol = plogReverse(plogSol)
                        cobra["SurrogateErrors"][
                            "OBJ" + str(obji) + "PLOG" + kernel
                        ].append((plogSol - yTrue[obji]) ** 2)
                        ...
                        cobra["SurrogateErrors"]["OBJ" + str(obji) + kernel].append(
                            (sol - yTrue[obji]) ** 2
                        )
                    else:
                        cobra["SurrogateErrors"][
            ...
            for coni in range(cobra["nConstraints"]):
                for kernel in cobra["RBFmodel"]:
                    if not cobra["cheapCon"][coni]:
                        ...
                        plogSol = cobra["GresPlogRescaledDivider"][coni] * plogSol
                        cobra["SurrogateErrors"][
                            "CON" + str(coni) + "PLOG" + kernel
                        ].append((plogSol - conTrue[coni]) ** 2)

                        surrogate = surrogateModels[kernel]["Constraints"]["Rescaled"][
                            coni
                        ]
                        sol = interpRBF(x, surrogate)
                        sol = cobra["GresRescaledDivider"][coni] * sol
                        cobra["SurrogateErrors"]["CON" + str(coni) + kernel].append(
                            (sol - conTrue[coni]) ** 2
                        )
                    else:
                        cobra["SurrogateErrors"][
        ...
        for i in range(1, len(cobra["hypervolumeProgress"])):
            if (
                cobra["hypervolumeProgress"][i] - cobra["hypervolumeProgress"][i - 1]
                > 0
            ):
                hvgrowing[i] = True

        hvgrowing[-2 * cobra["batch"] :] = (
            True  # besides the hypervolume improvement iterations, the results from the last two iterations are also taken into account
        )
            ...
            if not cobra["cheapObj"][obji]:
                for kernel in cobra["RBFmodel"]:
                    if (
                        np.sqrt(
                            np.mean(
                                np.array(tempErrors["OBJ" + str(obji) + kernel])[
                                    hvgrowing
                                ]
                            )
                        )
                    ...
                    if (
                        np.sqrt(
                            np.mean(
                                np.array(
                                    tempErrors["OBJ" + str(obji) + "PLOG" + kernel]
                                )[hvgrowing]
                            )
                        )
                        < minScore
            ...
            if not cobra["cheapCon"][coni]:
                for kernel in cobra["RBFmodel"]:
                    if (
                        np.sqrt(
                            np.mean(
                                np.array(tempErrors["CON" + str(coni) + kernel])[
                                    hvgrowing
                                ]
                            )
                        )
                    ...
                    if (
                        np.sqrt(
                            np.mean(
                                np.array(
                                    tempErrors["CON" + str(coni) + "PLOG" + kernel]
                                )[hvgrowing]
                            )
                        )
                        < minScore
    ...
    def updateInfoAndCounters(
        cobra, xNew, yNewEval, conNewEval, phase, surrogateModels
    ):
        ...
        for coni in range(cobra["nConstraints"]):
            ...
            plogGres, GresPlogRescaledDivider[coni] = rescale_constr(
                cobra["Gres"][:, coni]
            )
        ...
        for ci in range(cobra["nConstraints"]):
            if conNewEval[ci] <= 0:
                cobra["EPS"][ci] = np.maximum(
                    cobra["EPS"][ci] * (1 - cobra["epsilonLearningRate"]),
                    cobra["seqTol"] ** 0.5,
                )
            else:
                cobra["EPS"][ci] = np.minimum(
                    (cobra["epsilonLearningRate"] + 1) * cobra["EPS"][ci],
                    cobra["epsilonMax"][ci],
                )
    ...
    def trainSurrogates(cobra, pool):
        ...
        if pool:
            partialTrainSurrogate = partial(
                trainSurrogate,
                A=A,
                GresRescaled=GresRescaled,
                GresPlogRescaled=GresPlogRescaled,
                FresStandardized=FresStandardized,
                FresPlogStandardized=FresPlogStandardized,
                cheapConstr=cobra["cheapCon"],
                cheapObj=cobra["cheapObj"],
            )
            ...
            for kernel in kernels:
                surrogateModels[kernel] = trainSurrogate(
                    kernel,
                    A,
                    GresRescaled,
                    GresPlogRescaled,
    ...
    def combineBest(submins, cobra, surrogateModels, bestPredictor, fnCheap):
        ...
        for xi in submins_xs:
            ...
            solutions[i] = potentialSolution
            constraintPrediction = getConstraintPrediction(
                xi,
                surrogateModels,
                bestPredictor,
                cobra["nConstraints"],
                cobra["GresPlogRescaledDivider"],
                cobra["GresRescaledDivider"],
                fnCheap,
                cobra["cheapCon"],
                cobra["EPS"],
            )
        ...
        pfandNew_constraints = np.vstack(
            (cobra["Gres"][cobra["paretoFrontierFeasible"]], constraint_solutions)
        )
        ...
        if len(paretoDominantSolutions) == cobra["batch"]:
            ...
            for i in range(10000):
                comb = paretoDominantSolutions[
                    np.random.choice(
                        len(paretoDominantSolutions), size=cobra["batch"], replace=False
                    )
                ]
            ...
            for solution in paretoDominantSolutions:
                if any(
                    np.sum(np.equal(solution, bestComp), axis=1) == len(bestComp[0])
                ):
            ...
            for xi in submins_xs:
                potC = getEquallyImportantConstraintPrediction(
                    xi,
                    surrogateModels,
                    bestPredictor,
                    cobra["nConstraints"],
    ...
    def findSurrogateMinimum(cobra, surrogateModels, bestPredictor, pool):
        ...
        if cobra["batch"] == 1:
            ...
            gCOBRA_partial = partial(
                ...
                GresRescaledDivider=cobra["GresRescaledDivider"],
                fnCheap=fnCheap,
                cheapCon=cobra["cheapCon"],
                EPS=cobra["EPS"],
            )
            cons.append({"type": "ineq", "fun": gCOBRA_partial})
            compute_infill_criteria_score_partial = partial(
                compute_infill_criteria_score,
                lower=cobra["lower"],
                upper=cobra["upper"],
                surrogateModels=surrogateModels,
                bestPredictor=bestPredictor,
                nObj=cobra["nObj"],
                infillCriteria=cobra["infillCriteria"],
                FresPlogStandardizedStd=cobra["FresPlogStandardizedStd"],
                FresStandardizedStd=cobra["FresStandardizedStd"],
                FresPlogStandardizedMean=cobra["FresPlogStandardizedMean"],
                FresStandardizedMean=cobra["FresStandardizedMean"],
                currentHV=cobra["currentHV"],
                paretoFrontier=cobra["paretoFrontier"],
                ref=cobra["ref"],
                fnCheap=fnCheap,
                cheapObj=cobra["cheapObj"],
            )
        else:
            xStarts = computeStartPoints(
                cobra, cobra["computeStartingPoints"] * cobra["batch"]
            )
            xStarts = xStarts.reshape(
                (cobra["computeStartingPoints"], len(cobra["lower"]) * cobra["batch"])
            )
            batch_gCOBRA_partial = partial(
                batch_gCOBRA,
                batch=cobra["batch"],
                A=cobra["A"],
                lower=cobra["lower"],
        ...
        f = partial(
            pool_job,
            criteria_function=compute_infill_criteria_score_partial,
            lower=list(cobra["lower"]) * cobra["batch"],
            upper=list(cobra["upper"]) * cobra["batch"],
            cons=cons,
            seqFeval=cobra["seqFeval"],
            seqTol=cobra["seqTol"],
        )
        if pool:
        ...
        xNew, status = combineBest(
            submins[1:], cobra, surrogateModels, bestPredictor, fnCheap
        )
    ...
    def get_best_strategy(cobra, pool):
        ...
        if (
            b - a < cobra["seqTol"]
            and len(cobra["hypervolumeProgress"]) > 3 * evaluations_stuck_max
        ):
    ...
    while n < cobra["feval"]:
        ...
        if cobra["batch"] == 1:
            ...
            cobra = updateInfoAndCounters(
                cobra, xNew, yNew, cNew, phase, surrogateModels
            )
            xNew = np.array([xNew])
        else:
            ...
            for result in res:
                ...
                conNewEval.append(cNew)
                cobra = updateInfoAndCounters(
                    cobra, xNew[i], yNew, cNew, phase, surrogateModels
                )
                i += 1

        n = len(cobra["A"])

        cobra["optimizationTime"] = np.append(
            cobra["optimizationTime"], time.time() - ptm
        )

        if cobra["plot"]:
            print(
                time.time() - ptm, n, cobra["feval"], cobra["hypervolumeProgress"][-1]
            )
    ...
    if cobra["oneShot"]:
        ...
        outdir = (
            "batchresults/" + str(functionName) + "/batch" + str(cobra["batch"]) + "/"
        )
